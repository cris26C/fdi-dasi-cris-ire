...
class Agent:
    ...
    def receive_message(self, alias: str, role: str, content: str):
        self.memory.add_message(alias, role, content)

    async def response(self, alias: str, message: Optional[str] = None):
        # Descartar si el candado ya está adquirido (evita encolar mensajes repetidos)
        if self._locks[alias].locked():
            logger.warning(f"Respuesta para '{alias}' ya en curso, descartando mensaje.")
            return None

        async with self._locks[alias]:
            return await self._process_turn(alias, message)

    def _inject_stalemate_breaker_if_needed(self, alias: str):
        history = self.memory.get_history(alias)
        user_msgs = [m['content'] for m in history if m.get('role') == 'user']
        if len(user_msgs) >= 3 and len(set(user_msgs[-3:])) == 1:
            logger.warning(f"Bucle detectado con '{alias}'. Inyectando ruptura.")
            self.receive_message(alias, "user", "AVISO DEL SISTEMA: La negociación está bloqueada. Cambia tu estrategia AHORA o acepta la oferta.")

    async def _process_turn(self, alias: str, message: Optional[str] = None):
        if message:
            self.receive_message(alias, "user", message)

        is_first_contact = alias not in self._initiated_aliases
        self._initiated_aliases.add(alias)

        if not is_first_contact:
            self._inject_stalemate_breaker_if_needed(alias)

        resources = get_actual_resources_and_objectives()
        prompt_template = INITIAL_GREETING_SYSTEM_PROMPT if is_first_contact else NEGOTIATOR_SYSTEM_PROMPT
        
        system_prompt = prompt_template.format(
            agent_alias=alias,
            **resources # Asume que desempaquetas actual, objetivo, faltante, sobrante
        )
        ...
        tools_executed = await self.execute_tools(response, alias)
        content = getattr(response.message, 'content', None) or ''
        self.sync_memory(alias, tools_executed, content)
    ...
    def sync_memory(self, alias: str, tools_executed: list, content: str):
        description = content if content else '[Ninguna acción ejecutada]'
        
        for tool in tools_executed:
            nombre, args = tool['name'], tool['arguments']
            if nombre == "send_message_to_alias":
                description = f"Envié un mensaje a {args.get('alias')}: '{args.get('mensaje')}'"
            elif nombre == "send_package":
                description = f"Envié un paquete a {args.get('alias')}: {args.get('package')}"

        existing = self.memory.get_history(alias)
        if existing and existing[-1].get('role') == 'assistant' and existing[-1].get('content') == description:
            return

        self.memory.add_message(alias, "assistant", description)
    ...
    #             else:
    #                 raise
    async def execute_tools(self, response, alias: str):
        ...
        for tc in tool_calls:
            ...
            if 'alias' not in args: args['alias'] = alias
            
            if name in AVAILABLE_TOOLS:
                logger.info(f"Ejecutando '{name}' con: {args}")
                tool_res = await AVAILABLE_TOOLS[name](**args)
                tools_executed.append({"name": name, "arguments": args, "response": tool_res})
    ...
    async def make_response(self, messages: list, tools: list = None):
        ...
        for attempt in range(max_retries):
            try:
                logger.debug(f"Llamando a Ollama API (intento {attempt + 1}/{max_retries})")
                return await asyncio.wait_for(
                    client.chat(model=config.LLM_MODEL, messages=messages, tools=active_tools),
                    timeout=timeout
                )
            except asyncio.TimeoutError:
                logger.warning(f"Timeout llamando a Ollama (intento {attempt + 1}/{max_retries})")
            except Exception as e:
                logger.error(f"Error llamando a Ollama (intento {attempt + 1}/{max_retries}): {str(e)}")
            
            # Espera exponencial antes del siguiente reintento (1s, 2s, 4s...)
            if attempt < max_retries - 1:
                await asyncio.sleep(2 ** attempt)
                
        # Si sale del bucle, fallaron todos los intentos
        logger.error("Se agotaron los reintentos para comunicarse con Ollama.")
        raise Exception("Ollama falló tras múltiples intentos")
